...
MenuItem {
    ...
    BasicText {
        ...
        Connections {
            ...
            // Decoding finished: fill the lane, put the project offset on the
            // slider and hand the track to the player.
    ...
    Label {
        ...
        SliderWithField {
            ...
            onValueChanged: {
                ...
                // The player is updated on a timer: with `live: true` this fires
                // on every frame of the drag, and rebuilding the avfilter graph
                // that often stalls the audio device.
                playerOffsetTimer.restart();
            }

            Timer {
                id: playerOffsetTimer;
                // Each tick rewrites the aligned file, so it only fires once the
                // slider settles.
                interval: 400;
                onTriggered: root.invalidatePlayerAudio();
            }
        }
    }
