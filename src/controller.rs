...
pub struct Controller {
    ...
    /// Prepares the aligned audio for the preview player, off the UI thread.
    /// The url arrives in `external_audio_preview_ready`; nothing is written
...
impl Controller {
    ...
    /// Reports how the audio will come out with the chosen output extension.
    ///
    /// Returns JSON for the UI to build the preservation badge and, on conflict,
    /// offer switching the container before the export starts.
    fn prepare_external_audio_preview(&mut self) {
        ...
        // The preview covers the video timeline, so the file lines up with what
        ...
        // One file per session, overwritten whenever the offset changes: a single
        // path means the player is never left holding a stale file.
        let path = std::env::temp_dir().join("gyroflow_external_audio_preview.wav");
        // path_to_url handles what a plain format! does not: a Windows path needs
        // the drive letter after three slashes (file:///C:/...), otherwise it
        // lands in the host part of the url, and its spaces have to be encoded.
        let url = QString::from(filesystem::path_to_url(&path.to_string_lossy()));
        ...
        let finished = util::qt_queued_callback_mut(QPointer::from(self as &Self), move |this, ok: bool| {
            this.external_audio_preparing = false;
        ...
        self.external_audio_preparing = true;
        self.external_audio_preparing_changed();
