...
fn entry() {
    ...
    if !fpvflow_core::settings::contains("defaultCodec") {
        fpvflow_core::settings::set("defaultCodec", 0.into()); // default to H.264 on mobile
    }

    util::save_exe_location();
    let sdk_path = external_sdk::SDK_PATH.as_ref().map(|x| x.to_string_lossy().to_string()).unwrap_or_default();
    ::log::debug!("Executable path: {:?}", fpvflow_core::settings::try_get("exeLocation"));
    ...
    if cfg!(target_os = "linux") {
        ...
        fpvflow_core::gpu::wgpu::WgpuWrapper::list_devices();
    }
    ...
    engine.set_property("isSandboxed".into(), fpvflow_core::filesystem::is_sandboxed().into());
    ...
    engine.set_property("openFileOnStart".into(), QUrl::from(QString::from(fpvflow_core::filesystem::path_to_url(&open_file))).into());
