...
/// Writes a WAV holding the track already aligned to the video timeline.
///
/// The preview player has no way to shift an audio track: the ffmpeg build
/// Gyroflow ships is a reduced one, without `asetpts` or `adelay`. Rather than
/// depend on a filter that may not exist, the offset is baked into a temporary
/// file - the same [`build_segment`] the export uses, so what is heard matches
/// what will be written.
///
/// `duration_s` is the video duration: the file covers exactly that span, with
/// silence wherever the track does not reach.
pub fn write_preview_wav(track: &AudioTrack, duration_s: f64, path: &std::path::Path) -> std::io::Result<()> {
    use std::io::Write;

    let channels = track.channels.max(1);
    let sample_rate = track.sample_rate.max(1);
    let samples = build_segment(track, 0.0, duration_s);

    // 32-bit float keeps the source untouched for the formats this feature
    // targets, and every player handles WAVE_FORMAT_IEEE_FLOAT.
    const BITS: u16 = 32;
    ...
    f.write_all(&FORMAT_IEEE_FLOAT.to_le_bytes())?;
...
mod tests {
    ...
    #[test]
    fn preview_wav_is_playable_and_carries_the_offset() {
        // The preview file is what the player reads, so it has to be a valid
        // IEEE-float WAV - and it must already carry the offset, since the
        // player is given no way to shift it.
        ...
        // A marker one second in, to find again in the output.
        track.samples[sample_rate as usize] = 1.0;

        let dir = std::env::temp_dir();

        let no_offset = dir.join("gyroflow_test_preview_0.wav");
        write_preview_wav(&track, 2.0, &no_offset).expect("write preview");

        let bytes = std::fs::read(&no_offset).expect("read preview");
        assert_eq!(&bytes[0..4], b"RIFF", "not a RIFF file");
        assert_eq!(&bytes[8..12], b"WAVE", "not a WAVE file");
        // Format tag 3 is IEEE float; 1 would be integer PCM.
        assert_eq!(u16::from_le_bytes([bytes[20], bytes[21]]), 3, "not IEEE float");
        assert_eq!(u32::from_le_bytes([bytes[24], bytes[25], bytes[26], bytes[27]]), sample_rate,
                   "the sample rate was changed");

        let marker_at = |path: &std::path::Path| -> usize {
            let b = std::fs::read(path).unwrap();
            let samples: Vec<f32> = b[44..].chunks_exact(4)
                .map(|c| f32::from_le_bytes([c[0], c[1], c[2], c[3]]))
                .collect();
            samples.iter().position(|s| *s > 0.5).expect("marker not found in the preview")
        };
        assert_eq!(marker_at(&no_offset), sample_rate as usize, "marker moved without an offset");

        // With a positive offset the track starts further in, so the marker
        // arrives half a second earlier in the video timeline.
        track.offset_seconds = 0.5;
        let shifted = dir.join("gyroflow_test_preview_shift.wav");
        write_preview_wav(&track, 2.0, &shifted).expect("write shifted preview");
        assert_eq!(marker_at(&shifted), sample_rate as usize / 2,
                   "the offset was not baked into the preview file");

        let _ = std::fs::remove_file(no_offset);
    ...
    /// The preview file reaches the player as an url, and a Windows path only
    /// survives the trip when the drive letter follows three slashes. Built with
    /// two, the letter is parsed as the host, the player stops seeing a local
    /// file and the track silently fails to play - no error anywhere.
    #[test]
    fn a_windows_path_becomes_a_local_file_url() {
        let path = std::path::Path::new(r"C:\Users\Some One\Temp\preview.wav");
        let url = url::Url::from_file_path(path).expect("absolute path");

        assert!(url.as_str().starts_with("file:///C:/"), "got {url}");
        assert_eq!(url.host_str(), None, "the drive letter must not be the host");
        assert!(!url.as_str().contains(' '), "spaces have to be encoded: {url}");
        assert_eq!(url.to_file_path().unwrap(), path, "must survive the round trip");
    }
}
